...
const Sidebar: React.FC<SidebarProps> = ({ sidebarOpen, setSidebarOpen, uploadedFiles, selectedFiles, busy, error, handleFileSelect, handleUpload }) => (
	<aside className={`bg-gradient-to-b from-slate-800 to-slate-700 text-white w-64 p-3 flex flex-col transition-transform duration-300 fixed h-full z-20 border-r border-slate-600 shadow-xl ${sidebarOpen ? 'left-0' : '-left-64'}`}>
		<div className="flex items-center gap-6 mb-6">
			...
			<span className="font-bold text-xl tracking-wide mt-2">Menu</span>
		</div>
			...
			{selectedFiles.length > 0 && (
				<div className="mt-2">
					...
					<button
						...
						onClick={handleUpload}
						disabled={busy}
						className={`bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded ${busy ? "opacity-60 cursor-not-allowed" : ""
							}`}
					>
		...
		{error && <div className="fixed bottom-3 right-3 text-xs bg-rose-700 px-3 py-2 rounded">{error}</div>}
			...
			<ul className="list-disc pl-4">
				{uploadedFiles.length === 0 ? (
					...
					uploadedFiles.map((file, idx) => (
						<li key={idx} className="text-sm text-slate-200">{file.filename}</li>
					))
				)}
			</ul>
		</div>
